...
## 2026-08-03：用户端整体卡面 UI 首批

- 状态：`committed`；原始基准提交 `eae205e44bd72212c7334bb09d1045a67eb88aa6`，部署状态未验证。
- 用户端通过 `user-app/user-main` 与管理端隔离，普通内容卡片、导航、共享页头、筛选容器、空状态、分页和页脚统一为精灵卡片衍生的圆角、点阵、渐变和轻层叠风格。
- 首页公告、外部链接、数据概览、快速导航和数据声明已整体卡面化；数字、栏目及卡片标题使用项目既有官网同源 `MIANFEIZITI`，正文保留系统字体保证可读性；亮色与暗色材质独立适配。
- 精灵、技能、蛋组、性格、属性克制与打击面页面已接入共享用户页头；接口、路由、数据结构和服务端均未改动。
- 前端生产构建（含 PWA、SEO）、中文异常字符和 `git diff --check` 通过；尚未启动 DEV 做桌面/移动端与明暗主题的浏览器视觉验收。