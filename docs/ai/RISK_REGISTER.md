...
- 2026-07-26 线上自动导入风险控制：服务器向导默认仅使用生产库快照和隔离图片目录演练，`--apply` 需要发布包 ID 精确确认；写入前停止 PM2并保存独立 SQLite 恢复点，写入后检查完整性、外键、实体版本、素材清单和实际文件，异常自动恢复数据库与图片。已修复 Dev 验证报告存在 errors 仍返回成功的退出码漏洞，并对数值范围格式做语义归一。剩余风险：真实生产 Linux 的 PM2 应用名、健康地址、权限、磁盘空间和完整回滚路径尚未执行验证；首次上线必须先跑默认演练，保留 Release/会话/导入器备份，并在最终 `CONFIRM` 前人工核对关键页面。若素材清单缺失，脚本会保持服务停止，必须从会话恢复点人工处置。
